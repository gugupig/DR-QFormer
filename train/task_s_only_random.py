"""
TASK S Only Training: Fragment Ranking (Random-Init Q-Former).

This script trains TASK S (fragment ranking) using a randomly initialized Q-Former
without any pre-trained weights. This serves as a baseline to compare with XLM-RoBERTa-based Q-Former.

Key Differences from task_s_only.py:
=====================================
...
- Output: Knowledge-infused representations [32, 768] → FragmentRankingHead
...
@dataclass
class TaskSRandomConfig:
    """Configuration for TASK S with random-init Q-Former."""
    # Data
    ...
    save_dir: str = "./checkpoints/task_s_random"
...
class RandomQFormerDataset(Dataset):
    """
    Dataset for Random-Init Q-Former training (TASK S).
    
    Simplified compared to SmokingDataset:
    - Always uses pre-computed embeddings
    - Returns token-level query embeddings [seq_len, 768]
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        Returns:
            dict with keys:
                ...
                - evidence_ranking: list of (idx, score) tuples (teacher signals)
                - sample_id: str
        """
        ...
        # Evidence ranking (teacher signals)
...
def collate_random_qformer_batch(batch: List[Dict]) -> Dict:
    """
    Collate function for Random Q-Former with dynamic padding.
...
def compute_ranking_loss_random(
    ...
    if loss_type == "listnet":
        ...
        # Check for NaN in teacher distribution (can happen if all scores are equal)
        if torch.isnan(teacher_dist).any():
            # Fallback to uniform distribution over valid fragments
        ...
        # KL(teacher || pred) = sum(teacher * (log(teacher) - log(pred)))
        # Use F.kl_div with log_target=False (teacher is probability, pred is log-probability)
        loss = F.kl_div(
            log_pred_dist,
            teacher_dist,
            reduction='none',
        ...
        # Use MSE between normalized scores (only on valid fragments)
        loss = F.mse_loss(
    ...
    if torch.isnan(loss):
        print("⚠️  Warning: NaN loss detected, returning 0.0")
        return torch.tensor(0.0, device=ranking_logits.device, requires_grad=True)
    
    return loss


class TaskSRandomTrainer:
    """
    Trainer for TASK S with Random-Init Q-Former.
    """
    
    def __init__(self, config: TaskSRandomConfig):
        self.config = config
        ...
        # Initialize models
        print("="*80)
        print("Initializing TASK S Trainer (Random-Init Q-Former)")
        print("="*80)
    ...
    def train_step(self, batch: Dict) -> Dict[str, float]:
        ...
        loss = compute_ranking_loss_random(
    ...
    def plot_training_curves(self, save_path: Optional[str] = None):
        ...
        fig.suptitle('TASK S Training (Random Q-Former)', fontsize=16, fontweight='bold')
...
def main():
    """Main training loop."""
    config = TaskSRandomConfig()
    
    print("="*80)
    print("TASK S Training: Random-Init Q-Former (Baseline)")
    print("="*80)
    print("\nConfiguration:")
    ...
    # Initialize trainer
    trainer = TaskSRandomTrainer(config)
